...
typedef struct
{
	...
	signed long lst_pow_sum;
	
	signed long now_out;
	signed long lst_out;
	signed long now_velocity_xdt;
} _steepest_st;
...
void steepest_descend(signed long arr[],unsigned char len,_steepest_st *steepest,unsigned char step_num,signed long in);
...
//float Moving_Average(unsigned char item,unsigned char width_num,float in);
void Moving_Average(float moavarray[],//滤波数组 数组长度：len+1
										unsigned short len ,//滤波数据长度
										unsigned short *fil_cnt,//滤波元素号数标记（静态，用作存储）
										float in,//输入
										float *out //输出
										);
...
void fir_arrange_filter(float *arr,unsigned short len,unsigned char *fil_cnt,float in,float *arr_out);  //len<=255 len >= 3
