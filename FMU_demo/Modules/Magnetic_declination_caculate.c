#include "Magnetic_declination_caculate.h"
...
float _get_mag_declination(double lat, double lon)
{
  ...
  */
  ...
  int min_lat = (int)((int)(lat / SAMPLING_RES)* SAMPLING_RES);
  int min_lon = (int)((int)(lon / SAMPLING_RES)* SAMPLING_RES);
  ...
  */
  ...
  if (lat >= SAMPLING_MAX_LAT) {
    min_lat = (int)((int)(lat / SAMPLING_RES) * SAMPLING_RES - SAMPLING_RES);
  };
  ...
  if (lon >= SAMPLING_MAX_LON) {
    min_lon = (int)((int)(lon / SAMPLING_RES) * SAMPLING_RES - SAMPLING_RES);
  };
  
  /* find index of nearest low sampling point */
  unsigned min_lat_index = (unsigned)((-(SAMPLING_MIN_LAT) + min_lat)  / SAMPLING_RES);
  unsigned min_lon_index = (unsigned)((-(SAMPLING_MIN_LON) + min_lon)  / SAMPLING_RES);
...
{
  static float mag_declination = 0.0;
  if(Starnum_Healthstate == 2)
    mag_declination = _get_mag_declination(lat, lon);
  return (mag_declination*0.01745329251994f);
}
